...
input_list=("WG_cont" "9" "11" "13" "15" "17" "19")
prefix=927_norm_RFD_cutoff77_bs0.1kb
distance=501 # max distance between minus and plus feature 
inputdir=927_mapped-normRDF
...
cat $inputdir/ORI_$x\_$prefix\.bedgraph | awk   'BEGIN { OFS="\t" } {if ($2 > -1 ) {print $0} }' >  $inputdir/ORI_$x\_$prefix\_all.bedgraph &&
cat $inputdir/ORI_$x\_$prefix\_all.bedgraph | awk   'BEGIN { OFS="\t" } {if ($4 >= 0.3 ) {print $0} }' >  $inputdir/ORI_$x\_$prefix\_th0.3.bedgraph &&
cat $inputdir/ORI_$x\_$prefix\_all.bedgraph | awk   'BEGIN { OFS="\t" } {if ($7 == "+" && $5 <= -0.2 && $6 >= 0.2 ) {print $0} else if ($7 == "-" && $5 >= 0.2 && $6 <= -0.2) {print $0} }' >  $inputdir/ORI_$x\_$prefix\_th0.2P+M.bedgraph &&
cat $inputdir/ORI_$x\_$prefix\_all.bedgraph | awk   'BEGIN { OFS="\t" } {if ($7 == "+" && $5 <= -0.3 && $6 >= 0.3 ) {print $0} else if ($7 == "-" && $5 >= 0.3 && $6 <= -0.3) {print $0} }' >  $inputdir/ORI_$x\_$prefix\_th0.3P+M.bedgraph \
;done